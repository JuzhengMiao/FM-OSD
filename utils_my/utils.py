...
def visualize(img=torch.Tensor, pred_landmarks=None, gt_landmarks=None, ratio=0.005, draw_line=True):
    ...
    for i in range(num):
        ...
        if gt_landmarks is not None:
            gtemp = gt_landmarks[i]  
            
            draw.ellipse((gtemp[1]-15, gtemp[0]-15, gtemp[1]+15, gtemp[0]+15), fill = 'green')
                
        if pred_landmarks is not None:
            pretemp = pred_landmarks[i]
            
            draw.ellipse((pretemp[1]-15, pretemp[0]-15, pretemp[1]+15, pretemp[0]+15), fill = 'red')
        if draw_line and pred_landmarks is not None and gt_landmarks is not None:
            draw.line((pretemp[1],pretemp[0],gtemp[1],gtemp[0]), fill = 'yellow', width=5)
