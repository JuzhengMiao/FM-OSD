...
class ViTExtractor:
    ...
    def preprocess_local(self, image_path: Union[str, Path],
                   load_size: Union[int, Tuple[int, int]] = None, location = None) -> Tuple[torch.Tensor, Image.Image]:
        """
        """
        pil_image = Image.open(image_path).convert('RGB')
        # if load_size is not None:
        ...
        h, w, _ = pil_image_np.shape
        
        top = max(0, location[0] - load_size//2)
        right = min(w, location[1] + load_size//2)
        left = max(0, location[1] - load_size//2)
        down = min(h, location[0] + load_size//2)

        pil_image_np = pil_image_np[top:down, left:right, :]
        ...
        gt_local = [location[0] - top, location[1] - left]
        offset = [top, left]
        return prep_img, pil_image, gt_local, offset
    
    def preprocess_local_withfeature(self, image_path: Union[str, Path],
                   load_size: Union[int, Tuple[int, int]] = None, location = None, large_feature = None) -> Tuple[torch.Tensor, Image.Image]:
        """
        ...
        crop_feature = large_feature[:, :, top:down, left:right]
        ...
        gt_local = [location[0] - top, location[1] - left]
        offset = [top, left]
        return prep_img, pil_image, gt_local, offset, crop_feature
    
    def preprocess_local_random(self, image_path: Union[str, Path],
                   load_size: Union[int, Tuple[int, int]] = None, location = None, randamrange = 50) -> Tuple[torch.Tensor, Image.Image]:
        pil_image = Image.open(image_path).convert('RGB')
        # if load_size is not None:
        #     pil_image = transforms.Resize(load_size, interpolation=transforms.InterpolationMode.LANCZOS)(pil_image)
        pil_image_np = np.array(pil_image)  
        ...
        random_int0 = random.randint(-randamrange, randamrange)
        random_int1 = random.randint(-randamrange, randamrange)

        location0 = location[0] + random_int0
        location1 = location[1] + random_int1

        top = max(0, location0 - load_size//2)
        right = min(w, location1 + load_size//2)
        left = max(0, location1 - load_size//2)
        down = min(h, location0 + load_size//2)
    ...
    def preprocess_local_random_new(self, image_path: Union[str, Path],
                   load_size: Union[int, Tuple[int, int]] = None, location = None, randamrange = 50) -> Tuple[torch.Tensor, Image.Image]:
        pil_image = Image.open(image_path).convert('RGB')
        ...
        if (down - top) < load_size:  
            if down >= load_size:
                top = down - load_size
            else:
                down = top + load_size
        
        if (right - left) < load_size:
            if right >= load_size:
                left = right - load_size
            else:
                right = left + load_size
    ...
    def _log_bin(self, x: torch.Tensor, hierarchy: int = 2) -> torch.Tensor:
        ...
        bin_x = BINFunction.apply(avg_pools, B, sub_desc_dim, num_bins, self.num_patches[0], self.num_patches[1], hierarchy)
        torch.cuda.synchronize()
        bin_x = bin_x.flatten(start_dim=-2, end_dim=-1).permute(0, 2, 1).unsqueeze(dim=1)
        # Bx1x(t-1)x(dxh)
        return bin_x

    def extract_descriptors(self, batch: torch.Tensor, layer: List[int] = 11, facet: str = 'key',
